...
public sealed class IndexTests(IServerFixture serverFixture) : PlaywrightTests(serverFixture)
{
    ...
    public async Task CreateLibrary()
    {
        ...
        await Page.GetByRole(AriaRole.Button, new() { Name = "Scan" }).ClickAsync();

        await Page.GetByText("Tasks").ClickAsync();
        (await Page.TitleAsync()).Should().Be($"Tasks - {name} - Tubeshade");

        await Page.GetByRole(AriaRole.Cell, new() { Name = "Completed" }).WaitForAsync();
        ...
        await Page
            .GetByRole(AriaRole.Button, new() { Name = "Update sponsor segments" })
            .ClickAsync(new() { Timeout = 5_000 });
