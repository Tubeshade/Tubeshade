...
{
    private static readonly Channel<Guid> TaskChannel = Channel.CreateUnbounded<Guid>(
    ...
    internal static ChannelReader<Guid> Reader => TaskChannel.Reader;
    ...
    private void ConnectionOnNotification(NpgsqlConnection connection, NpgsqlNotificationEventArgs args)
    {
        ...
        var queued = TaskChannel.Writer.TryWrite(taskId);
        Trace.Assert(queued);
    }
}
